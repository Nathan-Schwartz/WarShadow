require(['jquery', 'gearData', 'windowCoreFunctions', 'updateData', 'arrayData', 'sync', 'htmlInjection', 'repairCalc', 'jqueryUI', 'printArmorTab'], function($, gData, wCore, upDat, arrayData, sync, inject, rCalc, jqueryUI, armorTab) {
  ...
  //SelectMenus
  var repairReCalc = function(event, ui) {
    rCalc.repairCalc();
    rCalc.rewardCalc();
  };
  ...
  $("#attachmentSelect1, #attachmentSelect2, #attachmentSelect3, #attachmentSelect4, #attachmentSelect5, #enemyVest1, #enemyHelmet1, #selectGData").selectmenu({
    change: function(event, ui) {
      upDat.updateData(0);
    }
  });
  $("#weaponSelect1, #weaponSelect2, #weaponSelect3, #weaponSelect4, #weaponSelect5").selectmenu({
    change: function(event, ui) {
      upDat.updateData(0);
      setTimeout(function() {
        $("#chart").slideDown();
